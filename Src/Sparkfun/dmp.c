...
ICM_20948_Status_e dmpInit() {
    ...
    uint8_t mstODRconfig = 0x02;                     // Set the ODR configuration to 1100/2^4 = 68.75Hz
    ...
    result = ICM_20948_enable_dlpf(&imu, ICM_20948_Internal_Acc | ICM_20948_Internal_Gyr, true);
    if (result > worstResult) worstResult = result;
    ...
    const unsigned char accScale[4] = {0x02, 0x00, 0x00, 0x00};
    ...
    const unsigned char accScale2[4] = {0x00, 0x02, 0x00, 0x00};
    ...
    result = inv_icm20948_set_gyro_sf(&imu, 8, myFSS.g);
...
ICM_20948_Status_e dmpPostInit() {
    ...
    retval = dmpEnableSensor(INV_ICM20948_SENSOR_GAME_ROTATION_VECTOR, true);
    ...
    retval = dmpSetODRrate(DMP_ODR_Reg_Quat6, 1);  // Set to 225Hz
    if (retval != ICM_20948_Stat_Ok) return retval;
